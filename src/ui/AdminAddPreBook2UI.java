...
public class AdminAddPreBook2UI {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					AdminAddPreBook2UI window = new AdminAddPreBook2UI();
	...
	private void initialize() {
		...
		JLabel step = new JLabel("Step2：选择增添数量");
		...
		ok.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				int isOk=JOptionPane.showConfirmDialog(null, "是否确定增添?", "增添已有书籍", JOptionPane.YES_NO_OPTION);
				if(isOk==JOptionPane.YES_OPTION){
					if(operator.addPreBook(bookNumm,Integer.parseInt(textField.getText())))
					{
					SuccessUI success=new SuccessUI(2);
