...
public class AdminChaAdminPsUI {
	...
	private void initialize() {
		...
		button.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				boolean isRight = operator.isAdminPasswordRight(operator.global_account, password1.getText());
                if(!isRight)
                {
                ...
                {
                	boolean ret = operator.chAdminPassword(operator.global_account, password2.getText());
                    if(ret)
                    {
